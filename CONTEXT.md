...
| 方案 (Scheme) | 一种可切换的输入法编码规则，共十一方案（双拼×5：小鹤/微软/搜狗/智能ABC/自然码，加全拼/注音/粤拼/仓颉/速成/五笔 86），含接口 {id, name, paradigm, codeOf, planOf, layout, activate}；注册于方案注册表 |
| 范式 (Paradigm) | 码的派生方式：py 派生（码是拼音的纯函数：双拼/全拼）vs 字表查询（码是 word 的查表函数：形码，已接入——仓颉/速成/五笔 86） |
| 数据包 (Data Pack) | 构建期从上游字典抽取的版本化紧凑 `{字: 码}` JSON（wubi86.v1 / cangjie5.v1 / zhuyin-tones.v1 / jyutping-tones.v1）；方案首次激活经 activate() 懒加载，内存缓存+失败重试；版本化文件名 + SW cache-first 为唯一持久层（不进 localStorage/ SHELL）；`_meta` 键内嵌出处与许可；速成/全拼/自然码无包；粤拼包经构建期简繁桥以简体为键（运行时零映射） |
...
| 课程数据 (Course Data) | 每方案一份（`js/courses.js`，schema v1）：五阶清单（键位图/间隔操练/池练习/错词）+ 易混对 + 七日挑战谓词；双拼五变体共用骨架，全拼为提速变体；注音/粤拼/仓颉/速成五阶与五笔 86 字根总表已全范式供给 |
